...
This validated pattern is modified version of Medical Diagnosis pattern. It includes the same functionality as the original pattern. The difference is that this solution additionally contains Node Feature Discovery Operator, which task is to detect hardware features and expose it as labels for this node. Red Hat OpenShift Serverless component is modified to assign AI workload to nodes with available AMX feature. Machine Learning model was quantized to int8 precision to improve efficiency without much accuracy loss.

Workflow::

* Node Feature Discovery operator labels nodes with AMX capabilities.
