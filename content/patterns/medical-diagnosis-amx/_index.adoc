...
links:
  ...
  bugs: https://github.com/hybrid-cloud-patterns/medical-diagnosis-amx/issues
ci: medicaldiag
---
...
This validated pattern is a modified version of the Medical Diagnosis pattern. It was extended to showcase *4th Gen Intel Xeon Scalable Processors* capabilities, especially *link:https://www.intel.com/content/www/us/en/products/docs/accelerator-engines/advanced-matrix-extensions/overview.html[Intel AMX]* that speeds up AI workloads.
The pattern is based on a demo implementation of an automated data pipeline for chest X-ray analysis that was previously developed by Red Hat for the US Department of Veteran Affairs. It includes the same
functionality as the original demonstration, but uses the GitOps framework to deploy the pattern including Operators, creation of namespaces, and cluster configuration.
...
Red Hat OpenShift Serverless component is modified to assign AI workload to nodes with available AMX feature.
...
* KNative Serving was modified to schedule pods with AI workload only on nodes with enabled AMX feature
...
* Node Feature Discovery operator to label nodes with AMX capabilities
...
[IMPORTANT]
====
Presently, the {med-pattern} - AMX does not have an edge component. Edge deployment capabilities are planned as part of the pattern architecture for a future release.
====
...
image::medical-diagnosis-amx/physical-network.png[link="/images/medical-diagnosis-amx/physical-network.png"]
