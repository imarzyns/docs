...
[NOTE]
====
The {med-pattern} - AMX does not have a dedicated hub or edge cluster.
====
...
Please read this section only if Validate Pattern was deployed using Operator. URL of Vault can be found by running command:
...
Modified version of Medical Diagnosis pattern requires to use on-prem object storage instead of AWS S3 (or other cloud equivalent) user can set up ceph rgw object storage. To communicate with its API user can utilize aws-cli. Installation instruction is available on https://docs.aws.amazon.com/cli/latest/userguide/getting-started-install.html#getting-started-install-instructions[Amazon website]
...
Clone repository with xray images and push them to the bucket 
...
Xray images are stored in the following path in repo:
