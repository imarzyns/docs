...
. Generate certificates and keys that will be used by httpbin application. (Optionally user can generate them manually, please refer to Istio official guide: https://istio.io/latest/docs/tasks/traffic-management/ingress/secure-ingress/#generate-client-and-server-certificates-and-keys). Run the following command:
+
[source,terminal]
----
./pattern.sh make prepare_certs
----
...
. (Optional) If cluster is behind proxy there can be a problem with unsealing Vault. To fix it run the following command (Instead of angle bracket values put your proxy information):
+
[source,terminal]
----
