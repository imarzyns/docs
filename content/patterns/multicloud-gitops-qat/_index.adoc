...
A service mesh is a dedicated infrastructure layer that you can add to your applications. It allows you to transparently add capabilities like observability, traffic management, and security, without adding them to your own code. Sail Operator serve as the foundation for Red Hat OpenShift Service Mesh 3.
...
The basic pattern has been extended by 4 components: Intel Device Plugin Operator, Node Feature Discovery Operator (NFD), Sail Operator and simple HTTP request and response application. Main purpose of this extension is to enable TLS handshaking and showcase how *{intel-qat}* accelerates cryptographic operations.

Intel Device Plugin Operator task is to expose QAT resources to Openshift layer, so it makes it easy to use, when creating deployments. After QAT resources are properly exposed Node Feature Discovery Operator is deployed. NFD manages the detection of hardware features and configuration in an OpenShift Container Platform cluster. It labels the nodes with hardware-specific information.

Sail Operator deploys Istio Service Mesh. It is basically an extra layer added on top of user's applications, and it adds capabilities like traffic management between services and security, without manually adding them to code. Istio in version deployed by Sail Operator allows for additional configuration, that is enabling *{intel-qat}* in cryptographic operations. The last added component is httpbin application, which is HTTP request and response service used for testing and showcase purposes. Workload idea was based on https://istio.io/latest/docs/tasks/traffic-management/ingress/secure-ingress/#configure-a-tls-ingress-gateway-for-a-single-host[Istio official instructions on how to setup secure TLS ingress gateway].

The appearance of qat as private key provider in Istio ingress gateway pod logs confirms that Intel QAT is used:
